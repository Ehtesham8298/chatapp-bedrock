...
const MODELS = [
  { id: 'us.anthropic.claude-opus-4-6-v1', name: 'Claude Opus 4.6', tag: 'Powerful' },
  { id: 'us.anthropic.claude-sonnet-4-6', name: 'Claude Sonnet 4.6', tag: 'Smart' },
];

const getSystemPrompt = (modelName) =>
  `You are ${modelName}, made by Anthropic, running via AWS Bedrock. When asked about your model or version, always say you are "${modelName}". Be helpful, respond clearly and concisely. Use markdown formatting when appropriate.`;
...
function App() {
  ...
  const handleSend = useCallback((text, attachments) => {
    sendMessage(text, attachments, {
      ...
      systemPrompt: systemPrompt || getSystemPrompt(selectedModel.name),
    });
